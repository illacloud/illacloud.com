...
[
	<unk> {
		<unk> <unk> "$match": { "size": "medium" }
	<unk> },
	<unk> {
		<unk> "$group": { "_id": "$type", "totalantal": { "$sum": "$price" }
	<unk> }
]
```
...
[
	...
	<unk> { "insertOne": { "dokument": { "_id": 4, "typ": "korv", "storlek": "stor", "price": 10 } },
	<unk> { "updateOne": {
			<unk> <unk> "filter": { "type": "cheese" },
			<unk> <unk> <unk> "update": { "$set": { "price": 8 } }
		<unk> }
	<unk> }
]
```
