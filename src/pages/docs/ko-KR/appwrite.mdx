...
You can also create custom SQL queries and run them within Illa. With the Appwrite integration in Illa, you can query, insert, update, and delete data from a Appwrite database using SQL commands. This makes it easy to build data-driven applications and dashboards that display real-time data from a Appwrite database.
...
| 주인          | 필수 | The URL or IP address for your database                                                                |
| 데이터베이스 ID   | 필수 | unique identifier assigned to each database created within Appwrite                                    |
| 프로젝트 ID     | 필수 | unique identifier that represents your entire Appwrite project                                         |
| API key     | 필수 | a secret token that serves as a form of authentication when making API requests to the Appwrite server |
...
| Method Name   | 설명                                                                                      |
| ------------- | --------------------------------------------------------------------------------------- |
| 방법            | Create a document, Get a document, Update a document, Delete a document, List documents |
| 컬렉션 ID        | a unique identifier assigned to a database collection                                   |
...
| 속성            | 필수    | 설명                                                              |
| ------------- | ----- | --------------------------------------------------------------- |
| 문서 ID         | 필수    | a unique identifier assigned to a database collection document  |
| 응답 데이터        | 선택 과목 | an object containing the fields and values of the new document. |
...
| ------------- | ----- | ------------------------------------------------------------------------------------------- |
| 문서 ID         | 필수    | a unique identifier assigned to a database collection document                              |
| 응답 데이터        | 선택 과목 | An object containing the updated fields and values that you want to assign to the document. |
...
| 속성         | 필수    | 설명                                                                            |
| ---------- | ----- | ----------------------------------------------------------------------------- |
| Filter     | 선택 과목 | define conditions to filter the documents based on specific fields and values |
| Order by   | 선택 과목 | specify the field by which the resulting documents should be sorted           |
| Limit      | 선택 과목 | restrict the number of documents returned in the result set                   |
