...
| 기본 URL       | 필수    | The URL or IP address for your database                                                |
| URL 매개변수     | 선택 과목 | provide additional information or modify the behavior of API requests                  |
| 헤더           | 선택 과목 | additional metadata about the API request or response                                  |
| 쿠키           | 선택 과목 | small pieces of data that are stored on the client-side and sent with each API request |
| 입증           | 선택 과목 | verifying the identity of the user or application making the API request               |
...
| 헤더            | additional metadata about the API request or response if not specified in connection setting                 |
