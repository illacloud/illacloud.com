...
Hugging Face Inference API を利用してStable Diffusion modelに接続するリソースとアクションを確立します。 `runwayml/stable-diffusion-v1-5` と `stabilityai/stable-diffusion-2-1` の2つのモデルを利用できます。 `runwayml/stable-diffusion-v1-5` と `stabilityai/stable-diffusion-2-1` の2つのモデルを利用できます。
