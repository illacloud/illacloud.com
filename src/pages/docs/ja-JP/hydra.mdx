...
desc: ILLA クラウドを使用して、Hydraデータにアクセスして操作します。 クエリ、操作、およびデータの表示ができるアプリを作成します。 クエリ、操作、およびデータの表示ができるアプリを作成します。 クエリ、操作、およびデータの表示ができるアプリを作成します。
...
Illa アカウントにサインインし、`**App**` ページで Illa Builder でプロジェクトを作成します。 をクリックし、ページの下部にある **`Action List`** に移動します。 `**new**`をクリックし、データベースリストから**`Hydra`**を選択します。 次に、下の「接続設定」で説明されている必要なパラメータを使用してデータベースに接続します。 `**new**`をクリックし、データベースリストから**`Hydra`**を選択します。 次に、下の「接続設定」で説明されている必要なパラメータを使用してデータベースに接続します。 `**new**`をクリックし、データベースリストから**`Hydra`**を選択します。 次に、下の「接続設定」で説明されている必要なパラメータを使用してデータベースに接続します。
...
| ポート                | 必須   | 接続に使用するサーバーホストのポート番号。 ポートを指定しない場合、デフォルトのポートは '5432' です。 ポートを指定しない場合、デフォルトのポートは '5432' です。 ポートを指定しない場合、デフォルトのポートは '5432' です。                                                                                                                 |
| データベース             | 必須   | データベースの名前                                                                                                                                                                                                                                   |
...
| CA 証明書             | 必須   | サーバーの身元を確認し、安全を確立するために使用されるデジタル証明書 クライアントとHydraクラスタの間の暗号化された接続 指定しない場合は、デフォルトの証明書を入力します。 指定しない場合は、デフォルトの証明書を入力します。 指定しない場合は、デフォルトの証明書を入力します。                                                                                                |
