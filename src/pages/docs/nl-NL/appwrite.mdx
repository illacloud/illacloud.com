---
title: Appwrite
crowdinRepo: https://crowdin.com/multilingual/illacloud-website/163?languages=nl&filter=basic&value=0
metaTitle: Appwrite Integration Doc Whether ILLA Cloud
desc: Leer hoe je verbinding en interactie kunt maken met de Appwrite database met behulp van ILLA Cloud.
tagCategory: doc_menu_appwrite_click
categoryName: 🔨 Integraties
categoryPriority: 3
postPriority: 1
---

## <h2 hidden>Wat is Appwrite?</h2>

Appwrite in Illa is een database-integratie waarmee u verbinding en interactie kunt maken met een Appwrite database. Appwrite is een populair open-source database management systeem dat bekend staat om zijn snelheid, betrouwbaarheid en gebruiksgemak.

Met de Appwrite integratie in Illa, kunt u de data van een Appwrite database opzoeken, invoegen, bijwerken en verwijderen met behulp van SQL commando's. U kunt ook aangepaste SQL-query's maken en uitvoeren binnen Illa. Dit maakt het makkelijk om data-driven applicaties en dashboards te bouwen die real-time gegevens weergeven uit een Appwrite database.

Deze handleiding schetst het proces van het maken van een Admin Panel met behulp van ILLA Builder en Appwrite in een paar eenvoudige stappen. ILLA is een laag programmeerplatform voor ontwikkelaars dat het mogelijk maakt interne hulpmiddelen snel te ontwikkelen en te implementeren. Het maakt het mogelijk om pagina's te maken door UI-componenten te slepen en te laten vallen, door verbinding te maken met elke database of API en door JavaScript te schrijven. Om meer te weten te komen over Appwrite, ga naar hun website via <https://Appwrite.io/> Laten we beginnen!

### Stel uw Back end in op Appwrite

Na het inloggen op uw Appwrite account, op het Appwrite dashboard, klik op `Create project` en stel de naam in op `First Project`.

![app_create_project](https://cdn.illacloud.com/official-website/img/docs/app_create_project.png)

Dan kunnen we naar de projectpagina navigeren. Appwrite biedt meerdere platforms voor het bouwen van uw app en integraties.

![app_platforms](https://cdn.illacloud.com/official-website/img/docs/app_platforms.png)

Hier kiezen we Web App als voorbeeld.

Na het aanmaken van ons web-app project kunnen we naar de databasepagina gaan omdat we alleen Appwrite gebruiken als een database-integratie tot nu toe.

![app_dashboard](https://cdn.illacloud.com/official-website/img/docs/app_dahsboard.png)

Daarna kunnen we database aanmaken en collecties aanmaken binnen de database

![app_create_database](https://cdn.illacloud.com/official-website/img/docs/app_create_database.png)
Vanaf hier kunnen we attributen en gegevens maken in de collectie.

![app_create_data](https://cdn.illacloud.com/official-website/img/docs/app_create_data.png)

Om documenten op basis van kenmerken op te halen, moeten we overeenkomstige indexen maken voor elke attribuut.

![app_create_index](https://cdn.illacloud.com/official-website/img/docs/app_create_index.png)

Deze database is nu klaar om te integreren met Illa.

### Maak Appwrite

Er zijn twee manieren om een bron in Illa te maken nadat je bent ingelogd op je Illa account.

-   Maak in Bronnen

Log in op je Illa account, selecteer `Resources` bovenaan de pagina, en klik op de `Create New` knop.

![external_resource](https://cdn.illacloud.com/official-website/img/docs/external_resource.png)

Selecteer `Appwrite` uit de database lijst.

![action_list](https://cdn.illacloud.com/official-website/img/docs/action_list.png)

Maak verbinding met de database met de vereiste parameters beschreven in `Connectie Instellingen` hieronder.

Klik op `Test Connection` om te zien of we met succes verbinding kunnen maken met de database. Zo ja, klik op `Save Resources`, anders controleer je de hostnaam, port, gebruikersnaam en wachtwoord correct.

Na het maken van een bron wordt de aangepaste Appwrite weergegeven zoals getoond.

![app_ex_ready](https://cdn.illacloud.com/official-website/img/docs/app_ex_ready.png)

-   Maak in Bouwer

Log in op je Illa account, maak een project aan in Illa Builder in de `App` pagina en navigeer naar de `Action List` onderaan de pagina. Klik op `new` en selecteer `Appwrite` uit de database lijst. Maak vervolgens verbinding met de database met de vereiste parameters beschreven in `Connectie Instellingen` hieronder.

Klik op `Test Connection` om te zien of we met succes verbinding kunnen maken met de database. Zo ja, klik op `Save Resources`, anders controleer je de hostnaam, port, gebruikersnaam en wachtwoord correct.

### Verbindingsinstellingen

Hier moeten we informatie verstrekken voor het verbinden met de Appwrite database.

![app_config](https://cdn.illacloud.com/official-website/img/docs/app_config.png)

| Eigenschappen   | vereist   | Beschrijving                                                                                                        |
| --------------- | --------- | ------------------------------------------------------------------------------------------------------------------- |
| naam            | verplicht | De naam voor resource bij het maken van acties in de ILLA.                                                          |
| Hostnaam        | verplicht | De URL of het IP-adres van uw database                                                                              |
| Database ID     | verplicht | unieke id toegewezen aan elke database gemaakt binnen Appwrite                                                      |
| Project ID      | verplicht | unieke id die uw hele Appwrite project vertegenwoordigt                                                             |
| API sleutel     | verplicht | een geheime token die dient als een vorm van authenticatie bij het indienen van API-verzoeken op de Appwrite server |

Om de vereiste `Host`, `Database ID`, `Project ID`, en `API key` te vinden in Appwrite, we gaan eerst naar de knop 'instellen' in de linker onderhoek van de projectpagina.

Kopiëren van de `project ID` voor `project ID` en `API Endpoint` voor `Host` in de Appwrite configuratie in Illa.

![app_setting](https://cdn.illacloud.com/official-website/img/docs/app_setting.png)

Dan kunnen we op de `Bekijk API Keys` knop op dezelfde pagina klikken. Als je nog geen API hebt aangemaakt voor dit project, Klik op de knop 'API-sleutel maken' en selecteer alleen de map 'Database', omdat dat de map is die we zullen gebruiken vanuit Appwrite.

![app_api_key](https://cdn.illacloud.com/official-website/img/docs/app_api_key.png)

Kopieer de `API Key Secret` voor `API key` in Illa configuratie. Het kan op deze twee plekken worden gekopieerd.

![app_key_secret](https://cdn.illacloud.com/official-website/img/docs/app_key_secret.png)

Wat we over hebben is de `Database ID` die we kunnen vinden in de `Database` pagina. Kopiëren en plakken in de Appwrite configuratie in Illa.

![app_database](https://cdn.illacloud.com/official-website/img/docs/app_database.png)

Het eindproduct zou er zo uit moeten zien.

![app_final_config](https://cdn.illacloud.com/official-website/img/docs/app_final_config.png)

### Acties aanmaken

We hebben een Appwrite bron gemaakt, we kunnen de actie toevoegen door de Appwrite van de actie lijst te selecteren en te kiezen voor de knop 'Maak actie'.

![app_resource_list](https://cdn.illacloud.com/official-website/img/docs/app_resource_list.png)

Nu hebben we de Appwrite server toegevoegd als actie aan onze bouwpagina.

![app](https://cdn.illacloud.com/official-website/img/docs/app.png)

### Appwrite configureren

#### Overzicht.

| Methode Naam    | Beschrijving                                                                                                         |
| --------------- | -------------------------------------------------------------------------------------------------------------------- |
| Methode         | Een document maken, een document ophalen, een document bijwerken, verwijderen van een document, lijst met documenten |
| Verzameling ID  | een unieke identifier toegewezen aan een database-collectie                                                          |
| Transformator   | data omzetten naar de stijl die je leuk vindt met behulp van JavaScript                                              |

#### Een document aanmaken

maak een nieuw document aan in een collectie

 Input 

| Eigenschappen   | vereist   | Beschrijving                                                     |
| --------------- | --------- | ---------------------------------------------------------------- |
| Document ID     | verplicht | een unieke id toegewezen aan een database collectie document     |
| Gegevens        | optioneel | een object met de velden en waarden van het nieuwe document.     |

Bijvoorbeeld, het maken van een nieuw document in een collectie met twee velden - `name` en `age`, je kan 1 gebruiken als `document ID` en `{{ {"Name": "David", "Age": 18}}}` voor `data`.

 Uitvoer 

een responsobject dat de ID van het nieuwe document bevat, evenals extra metadata zoals de datum en tijd van de creatie.

![app_create_code](https://cdn.illacloud.com/official-website/img/docs/app_create_code.png)

#### Document ophalen

een specifiek document ophalen uit een collectie in de database

 Input 

| Eigenschappen  | vereist   | Beschrijving                                                    |
| -------------- | --------- | --------------------------------------------------------------- |
| Document ID    | verplicht | een unieke id toegewezen aan een database collectie document    |

We kunnen het document bijvoorbeeld krijgen met id `1`.

 Uitvoer 

een antwoord object dat de gegevens van het gevraagde document bevat

![app_get_data](https://cdn.illacloud.com/official-website/img/docs/app_get_data.png)

Gebruik `{{Appwrite1.data}}` om alle geretourneerde waarden te krijgen.

![app_get_code](https://cdn.illacloud.com/official-website/img/docs/app_get_code.png)

#### Een document bijwerken

de gegevens van een bestaand document in een collectie bijwerken.

 Input 

| Eigenschappen   | vereist   | Beschrijving                                                                                 |
| --------------- | --------- | -------------------------------------------------------------------------------------------- |
| Document ID     | verplicht | een unieke id toegewezen aan een database collectie document                                 |
| Gegevens        | optioneel | Een object met de bijgewerkte velden en waarden die u wilt toewijzen aan het document.       |

Bijvoorbeeld, om een document in een verzameling bij te werken met behulp van de `updateDocument` methode. je kan `1` voor document id en `{{{"Name":"Fred"}}}` plaatsen voor gegevens.

 Uitvoer 

een antwoord object dat de gegevens van het gevraagde document bevat

![app_update)data](https://cdn.illacloud.com/official-website/img/docs/app_update_data.png)

#### Verwijder een document

verwijder een specifiek document uit een collectie in de database

 Input 

| Eigenschappen  | vereist   | Beschrijving                                                    |
| -------------- | --------- | --------------------------------------------------------------- |
| Document ID    | verplicht | een unieke id toegewezen aan een database collectie document    |

We kunnen het document met id `1` verwijderen.

 Uitvoer 

een reactie-object dat het met succes verwijderen van het document bevestigt.

![app_delete_data](https://cdn.illacloud.com/official-website/img/docs/app_delete_data.png)

#### Lijst document

meerdere documenten ophalen uit een collectie in de database

 Input 

| Eigenschappen | vereist   | Beschrijving                                                                              |
| ------------- | --------- | ----------------------------------------------------------------------------------------- |
| Filteren      | optioneel | definieer voorwaarden om documenten te filteren gebaseerd op specifieke velden en waarden |
| Sorteer op    | optioneel | specificeer het veld waarop de resulterende documenten moeten worden gesorteerd           |
| Limiet        | optioneel | beperken van het aantal documenten geretourneerd in de resultaatset                       |

Bijvoorbeeld, het tonen van alle documenten met naam = James.

 Uitvoer 

Een reeks documenten die aan het filter voldoen.

![app_list_data](https://cdn.illacloud.com/official-website/img/official-site/intergration/app_list_data.png)

Gebruik `{{Appwrite1.data[0].documents}}` om de waarde terug te krijgen.

![app_list_code](https://cdn.illacloud.com/official-website/img/docs/app_list_code.png)
