---
title: Appwrite
crowdinRepo: https://crowdin.com/multilingual/illacloud-website/163?languages=no&filter=basic&value=0
metaTitle: Skriv inn IntegrasjonsDoc ∙ILLA Cloud
desc: Lær hvordan du kobler til og samhandler med Appwrite database ved å bruke ILLA Cloud.
tagCategory: doc_menu_appwrite_click
categoryName: 🔨 Integrasjoner
categoryPriority: 3
postPriority: 1
---

## <h2 hidden>Hva er Appwrite?</h2>

Appwrite in Illa is a database integration that allows you to connect and interact with a Appwrite database. Appwrite er et populært relasjonssystem for database med åpen kildekode som er kjent for sin hastighet, pålitelighet og bruksfrihet.

Med integrasjonen Appwrite i Illa, kan du spørre, innstille, oppdatere og slette data fra en skrivedatabase ved å bruke SQL-kommandoer. Du kan også opprette egendefinerte SQL-spørringer og kjøre dem i Illa. Dette gjør det enkelt å bygge data-drevne programmer og dashbord som viser sanntidsdata fra en Appwrite database.

Denne veiledningen viser prosessen med å lage et Admin-panel ved hjelp av ILLA Builder og Appwrite i noen enkle trinn. ILLA er en lavkode-plattform for utviklere som gjør det mulig å raskt utvikle og ta i bruk interne verktøy. Det åpner for å opprette sider ved å dra og slippe UI komponenter, koble til database eller API, og skrive JavaScript. For å lære mer om Appwrite, besøk sin nettside på <https://Appwrite.io/> La oss begynne!

### Sett opp Back end på Appwrite

Etter å ha logget deg på Appwrite account på Appwrite dashboard, klikk `Create project` og la navnet være å `First Project`.

![app_create_project](https://cdn.illacloud.com/official-website/img/docs/app_create_project.png)

Da kan vi navigere til prosjektsiden. Appwrite tilbyr flere plattformer for bygging av appen din og integrasjoner.

![app_platforms](https://cdn.illacloud.com/official-website/img/docs/app_platforms.png)

Her velger vi Web App som et eksempel.

Etter at vi har opprettet nett-appprosjektet vårt, kan vi gå til databasesiden siden siden vi bare bruker Appwrite som en databaseintegrasjon så langt.

![app_dashboard](https://cdn.illacloud.com/official-website/img/docs/app_dahsboard.png)

Da kan vi lage databasen og opprette samlinger i databasen

![app_create_database](https://cdn.illacloud.com/official-website/img/docs/app_create_database.png)
herfra kan vi opprette attributter og data i samlingen.

![app_create_data](https://cdn.illacloud.com/official-website/img/docs/app_create_data.png)

For å hente dokumenter fra attributter, må vi gjøre tilsvarende indekser for hvert attributt.

![app_create_index](https://cdn.illacloud.com/official-website/img/docs/app_create_index.png)

Denne databasen er nå klar til å integrere med Illa.

### Opprett Appwrite

Det er to måter å lage en ressurs i Illa etter å ha logget på din Illa-konto.

-   Opprett i ressurser

Logg deg på din Illa konto, velg `Resources` på toppen av siden, og klikk `Opprett ny` knappen.

![external_resource](https://cdn.illacloud.com/official-website/img/docs/external_resource.png)

Velg `Appwrite` fra databaselisten.

![action_list](https://cdn.illacloud.com/official-website/img/docs/action_list.png)

Koble til databasen med de nødvendige parametrene beskrevet i "Tilkoblingsinnstillinger" nedenfor.

Klikk `Test Connection` for å se om vi har lykkes med å koble til databasen. Hvis ja, klikk `Lagre ressurser`, ellers dobbeltsjekk vertsnavn, port, brukernavn og passord er riktig.

Etter oppretting av en ressurs, vil Appwrite vises som vist.

![app_ex_ready](https://cdn.illacloud.com/official-website/img/docs/app_ex_ready.png)

-   Opprett i bygger

Logg deg på din Illa konto, opprett et prosjekt i Illa Builder på `App`-siden, og naviger til 'Handling-listen' nederst på siden. Klikk `new`, og velg `Appwrite` fra databaselisten. Deretter kan du koble til databasen med nødvendige parametere beskrevet i "Tilkoblingsinnstillinger" nedenfor.

Klikk `Test Connection` for å se om vi har lykkes med å koble til databasen. Hvis ja, klikk `Lagre ressurser`, ellers dobbeltsjekk vertsnavn, port, brukernavn og passord er riktig.

### Tilkobling innstillinger

Her må vi gi informasjon for å koble til Appwrite database.

![app_config](https://cdn.illacloud.com/official-website/img/docs/app_config.png)

| Egenskaper     | Påkrevd      | Beskrivelse                                                                                                       |
| -------------- | ------------ | ----------------------------------------------------------------------------------------------------------------- |
| Navn           | obligatorisk | Navnet på ressursen når det skapes handlinger i ILLA.                                                             |
| Vert           | obligatorisk | URL-adressen eller IP-adressen for databasen din                                                                  |
| Database ID    | obligatorisk | unik identifikator som er tilordnet hver database opprettet i Appwrite                                            |
| Prosjekt ID    | obligatorisk | unik identifikator som representerer hele Appwrite prosjektet                                                     |
| API nøkkel     | obligatorisk | et hemmelig token som fungerer som et slags autentisering når du oppretter API forespørsler til Appwrite serveren |

For å finne den nødvendige `vert`, `Database ID`, `Project ID`, og `API key` i Appwrite, vi går først til "innstilling" knappen nederst til venstre på prosjektsiden.

Kopierer `prosjekt ID` for `prosjekt ID` og `API Endpoint` for `Host` i Appwrite konfigurasjonen i Illa.

![app_setting](https://cdn.illacloud.com/official-website/img/docs/app_setting.png)

Da kan vi klikke på `Vis API-nøkler` på samme side. Hvis du ikke har laget noen API for dette prosjektet, klikk `Opprett API Key` knappen og velg kun `Database` mappen siden det er det vi vil bruke fra Appwrite.

![app_api_key](https://cdn.illacloud.com/official-website/img/docs/app_api_key.png)

Kopier `API Key Secret` for `API key` i Illa konfigurasjon. Den kan kopieres for disse to søkene.

![app_key_secret](https://cdn.illacloud.com/official-website/img/docs/app_key_secret.png)

Det vi har igjen er `database ID` som vi finner i `Database` siden. Kopiering og lim inn på appwrite konfigurasjonen i Illa.

![app_database](https://cdn.illacloud.com/official-website/img/docs/app_database.png)

Det endelige produktet skal se omtrent slik ut.

![app_final_config](https://cdn.illacloud.com/official-website/img/docs/app_final_config.png)

### Opprett handlinger

Vi har opprettet en Appwrite resource, vi kan legge til handlingen ved å velge Appwrite fra handling-listen og velge `Create action`-knappen.

![app_resource_list](https://cdn.illacloud.com/official-website/img/docs/app_resource_list.png)

Nå har vi lagt til Appwrite serveren som en handling på byggesiden vår.

![app](https://cdn.illacloud.com/official-website/img/docs/app.png)

### Konfigurer Appwrite

#### Oversikt

| Metodenavn (Automatic Translation)   | Beskrivelse                                                                                         |
| ------------------------------------ | --------------------------------------------------------------------------------------------------- |
| Metode                               | Opprett et dokument, Få et dokument, oppdater et dokument, Slett et dokument, Liste over dokumenter |
| Samling ID                           | en unik identifikator som er tilordnet en databasesamling                                           |
| Transformator                        | transformerer data til stilen du ønsker å bruke JavaScript                                          |

#### Opprett et dokument

opprett et nytt dokument i en samling

 Input 

| Egenskaper  | Påkrevd      | Beskrivelse                                                           |
| ----------- | ------------ | --------------------------------------------------------------------- |
| Dokument ID | obligatorisk | en unik identifikator som er knyttet til et databasesamlingsdokument  |
| Data        | valgfritt    | et objekt som inneholder feltene og verdiene i det nye dokumentet.    |

For eksempel for å opprette et nytt dokument i en samling med to felt - 'navn' og 'age' du kan bruke 1 som `document ID` og `{{ {"Name": "David", "Age": 18}}` for `data`.

 Utgang 

et svarobjekt som inkluderer ID til det nyopprettede dokumentet, i tillegg til ekstra metadata som for eksempel dato og klokkeslett.

![app_create_code](https://cdn.illacloud.com/official-website/img/docs/app_create_code.png)

#### Få et dokument

hente et spesifikt dokument fra en samling i databasen

 Input 

| Egenskaper  | Påkrevd      | Beskrivelse                                                          |
| ----------- | ------------ | -------------------------------------------------------------------- |
| Dokument ID | obligatorisk | en unik identifikator som er knyttet til et databasesamlingsdokument |

Vi kan for eksempel få dokumentet med id `1`.

 Utgang 

et svarobjekt som omfatter dataene til det forespurte dokumentet

![app_get_data](https://cdn.illacloud.com/official-website/img/docs/app_get_data.png)

Bruk "{{Appwrite1.data}}" for å få alle verdier som returneres.

![app_get_code](https://cdn.illacloud.com/official-website/img/docs/app_get_code.png)

#### Oppdater et dokument

oppdatere dataene til et eksisterende dokument i en innsamling.

 Input 

| Egenskaper  | Påkrevd      | Beskrivelse                                                                                       |
| ----------- | ------------ | ------------------------------------------------------------------------------------------------- |
| Dokument ID | obligatorisk | en unik identifikator som er knyttet til et databasesamlingsdokument                              |
| Data        | valgfritt    | Et objekt som inneholder de oppdaterte feltene og verdiene som du ønsker å tilordne dokumentet.   |

For eksempel for å oppdatere et dokument i en samling ved hjelp av metoden `updateDocument` du kan angi `1` for dokument-ID og `{{{"Name":"Fred"}}}` for data.

 Utgang 

et svarobjekt som omfatter dataene til det forespurte dokumentet

![app_update)data](https://cdn.illacloud.com/official-website/img/docs/app_update_data.png)

#### Slett et dokument

slette et spesifikt dokument fra samlingen i databasen

 Input 

| Egenskaper  | Påkrevd      | Beskrivelse                                                          |
| ----------- | ------------ | -------------------------------------------------------------------- |
| Dokument ID | obligatorisk | en unik identifikator som er knyttet til et databasesamlingsdokument |

For eksempel kan vi slette dokumentet med id `1`.

 Utgang 

et svarobjekt som bekrefter vellykket sletting av dokumentet.

![app_delete_data](https://cdn.illacloud.com/official-website/img/docs/app_delete_data.png)

#### Vis dokument

hente flere dokumenter fra en samling i databasen

 Input 

| Egenskaper     | Påkrevd   | Beskrivelse                                                                      |
| -------------- | --------- | -------------------------------------------------------------------------------- |
| Filtrer        | valgfritt | angi vilkår for filtrering av dokumentene basert på spesifikke felter og verdier |
| Sorter etter   | valgfritt | angi feltet som de resulterende dokumentene bør sorteres etter                   |
| Grense         | valgfritt | begrense antallet dokumenter som er returnert i resultatsettet                   |

For eksempel, for å liste opp alle dokumenter med Name = James.

 Utgang 

En rekke dokumenter som tilfredsstiller filteret.

![app_list_data](https://cdn.illacloud.com/official-website/img/official-site/intergration/app_list_data.png)

Bruk "{{Appwrite1.data[0].documents}}" for å komme tilbake i verdi.

![app_list_code](https://cdn.illacloud.com/official-website/img/docs/app_list_code.png)
