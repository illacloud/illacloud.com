...
点击“测试连接”查看我们是否可以成功连接到数据库。 如果是，请单击\`保存资源'，否则请重新检查主机名、端口、用户名和密码是否正确。
...
登录到您的Illa帐户，在`App`页面的Illa Builder创建一个项目 并导航到页面底部的“行动列表”。 点击`new`，然后从数据库列表中选择 `Appwrite` 。 然后，用下面`链接设置`描述的参数连接到数据库。
...
然后我们可以点击同一页面上的 `View API Keys` 按钮。 如果您没有为此项目创建API， 点击 `Create API Key` 按钮，只选择 `Database` 文件夹，因为这是我们将从 Appwrite 中使用的。
