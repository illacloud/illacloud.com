...
Illa 中的 AppWrite 是一个数据库集成，允许您连接并与 AppWrite 数据库交互。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。 Appwrite 是一个受欢迎的开放源码关系数据库管理系统，它以其速度、可靠性和易用性而著称。

在Illa中使用 Appwrite 集成，您可以通过 SQL 命令查询、插入、更新和删除应用写入数据库中的数据。 您也可以创建自定义 SQL 查询并在Illa中运行它们。 这就很容易建立数据驱动的应用程序和仪表板，从应用写入数据库中显示实时数据。 您也可以创建自定义 SQL 查询并在Illa中运行它们。 这就很容易建立数据驱动的应用程序和仪表板，从应用写入数据库中显示实时数据。 您也可以创建自定义 SQL 查询并在Illa中运行它们。 这就很容易建立数据驱动的应用程序和仪表板，从应用写入数据库中显示实时数据。 您也可以创建自定义 SQL 查询并在Illa中运行它们。 这就很容易建立数据驱动的应用程序和仪表板，从应用写入数据库中显示实时数据。

本教程概述了使用 ILLA Builder 和 Appwrite 在几个简单步骤创建管理面板的过程。 ILLA Cloud是一个低码的开发者平台，能够快速开发和部署内部工具。 它允许通过拖放UI组件创建页面，并连接到任何数据库或API，以及写入 JavaScript。 要了解更多关于 Appwrite.io/(https://appwrite.io/) 让我们开始吧！ ILLA Cloud是一个低码的开发者平台，能够快速开发和部署内部工具。 它允许通过拖放UI组件创建页面，并连接到任何数据库或API，以及写入 JavaScript。 要了解更多关于 Appwrite.io/(https://appwrite.io/) 让我们开始吧！ ILLA Cloud是一个低码的开发者平台，能够快速开发和部署内部工具。 它允许通过拖放UI组件创建页面，并连接到任何数据库或API，以及写入 JavaScript。 要了解更多关于 Appwrite.io/(https://appwrite.io/) 让我们开始吧！ ILLA Cloud是一个低码的开发者平台，能够快速开发和部署内部工具。 它允许通过拖放UI组件创建页面，并连接到任何数据库或API，以及写入 JavaScript。 要了解更多关于 Appwrite.io/(https://appwrite.io/) 让我们开始吧！
...
登录您的 Illa 帐户，在`**App**` 页面中创建一个项目 并导航到页面底部的 **`Action List` **。 点击`**新**`，然后从数据库列表中选择 **`AppWrite`** 。 然后，用下面`链接设置`描述的参数连接到数据库。 点击`**新**`，然后从数据库列表中选择 **`AppWrite`** 。 然后，用下面`链接设置`描述的参数连接到数据库。
...
要找到所需的 **`Host`**、 **`Database ID`**、 **`Project ID`\*\*** 和 **`API key`** 在 Appwrite， 我们首先到项目页面左下角的 `**setting**` 按钮。

将`project ID` 复制为 **`project ID` ** 和 `API 端点` 到 \*\*`Host` 在 Illa 的Appwrite 配置中。

![app_setting](https://cdn.illacloud.com/official-website/img/docs/app_setting.png)

然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 如果您没有为此项目创建API， 点击 **`Create API Key`** 按钮，只选择 `Database` 文件夹，因为这是我们将从 Appwrite 使用的。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 如果您没有为此项目创建API， 点击 **`Create API Key`** 按钮，只选择 `Database` 文件夹，因为这是我们将从 Appwrite 使用的。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 如果您没有为此项目创建API， 点击 **`Create API Key`** 按钮，只选择 `Database` 文件夹，因为这是我们将从 Appwrite 使用的。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 然后我们可以点击同一页面上的 **`查看 API Keys`** 按钮。 如果您没有为此项目创建API， 点击 **`Create API Key`** 按钮，只选择 `Database` 文件夹，因为这是我们将从 Appwrite 使用的。

![app_api_key](https://cdn.illacloud.com/official-website/img/docs/app_api_key.png)

在Illa配置中复制 `API 密钥秘书` 。 可以复制这两个地点。 可以复制这两个地点。 可以复制这两个地点。 可以复制这两个地点。

![app_key_secret](https://cdn.illacloud.com/official-website/img/docs/app_key_secret.png)

我们剩下的是**`Database`页面** 。 复制并粘贴到Illa的Appwrite 配置中。
...
我们创建了一个 AppWrite 资源，我们可以从动作列表中选择 AppWrite 并选择 **`Create action` ** 按钮来添加动作。

![app_resource_list](https://cdn.illacloud.com/official-website/img/docs/app_resource_list.png)

现在我们已经将 AppWrite 服务器添加到我们的构建页面作为动作。
...
例如，在集合中创建一个包含两个字段的新文档 - **`name`** 和 **`age`** \* 你可以使用 1 作为 **`document ID` ** 和 `{□ {"Name": "David", "Age": 18}}} 为 **`data\`\*\*。
...
使用{{appwrite1.data[0].documents}}获取返回值。
