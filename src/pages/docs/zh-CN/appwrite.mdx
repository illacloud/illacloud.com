...
在Appwrite的不同服务中，您可以找到用户认证和账户管理、用户偏好、数据库和存储持久性、云功能、本地化、图像操作等等。 
...
| 属性     | 必填项 | 描述                                                         |
| ------ | --- | ---------------------------------------------------------- |
| 名称     | 必填  | 在ILLA中创建Action时可以看到的资源名称。 您可以自定义名称以便于识别。 您可以自定义一个名称以便于识别。  |
| 主机     | 必填  | 主机名或IP                                                     |
| 数据库 ID | 必填  | 数据库 ID                                                     |
| 项目 ID  | 必填  | 项目 ID                                                      |
| Api 密钥 | 必填  | 托克                                                         |
...
创建操作后，请选择一个方法。 我们提供了以下方法：列出文档、创建文档、更新文档、获取文档并删除文档。 每种方法的配置如下。 
...
| 属性   | 必填项 | 描述                                                                                                                                                        |
| ---- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 收藏ID | 必填  | 输入或从数据库中选择收藏ID                                                                                                                                            |
| 筛选器  | 可选的 | 支持多个过滤器。 支持多个过滤器。过滤器由Key、Value和Condition组成。1. 1. key：输入一个属性2. condition：选择一个条件，包括equal, notEqual, lessThan, lessThanEqual, greaterThan等。3.value：输入值 值：输入值 |
| 排序方式 | 可选的 | 支持按多个属性排序。 1. 输入一个属性到排序方式 2。 选择升序(默认) 或降序排列                                                                                                               |
| 限制   | 可选的 | 返回的记录数量                                                                                                                                                   |

下面举例说明了如何在Action中使用组件中输入的数据：

1.  在框架中添加一个输入组件，以便用户能够输入他们想要找到的文档id。  
2.  添加过滤器：将属性字段设置为 $id 并在值字段中输入 {{ input1.value }}。 

<img src={require('@/img/docs/assemble/appwrite6.png').default} alt="" />

输出

检查左侧面板上的数据结构。 检查左侧面板上的数据结构。例如，使用 `{{list.data[0].documents[0].id}}`（`list` 是Action的名称）来获取第一个文档的id。 
...
| 收藏ID  | 输入或从数据库中选择收藏ID                                        |
| 文档 ID | 输入要更新的文档ID                                            |
| 数据    | 包含 {{ }} 的对象。 包含在 {{ }} 中的对象。 例如，{{ {“Key”：“Value”}}} |
...
产出数据如下。 输出数据如下。 使用 {{get.data[0]}}（get是Action的名称） 获取文档对象。
