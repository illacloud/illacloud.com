...
在连接Appwrite资源之前，请确保您已在本地计算机或云提供商上部署了Appwrite。[点击这里]([https://appwrite.io/docs/installation](https://appwrite.io/docs/installation))了解有关Appwrite安装的更多信息。
...
| Filter | Optional | 支持多个过滤器。
过滤器由Key、Value和Condition组成。
1. key：输入一个属性
2. condition：选择一个条件，包括equal, notEqual, lessThan, lessThanEqual, greaterThan等。
3.value：输入值 |
| Order by | Optional | 支持按多个属性排序。
1.输入一个属性来排序
2.选择asc（默认）或desc |
