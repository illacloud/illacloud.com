...
点击“测试连接”查看我们是否可以成功连接到数据库。 如果是，请单击\`保存资源'，否则请重新检查主机名、端口、用户名和密码是否正确。 如果是，请单击\`保存资源'，否则请重新检查主机名、端口、用户名和密码是否正确。
...
然后我们可以点击同一页面上的 `View API Keys` 按钮。 然后我们可以点击同一页面上的 `View API Keys` 按钮。 如果您没有为此项目创建API， 点击 `Create API Key` 按钮，只选择 `Database` 文件夹，因为这是我们将从 Appwrite 中使用的。
