...
desc: 使用 ILLA Cloud访问并使用您的 Hydra 数据。 创建可以查询、操作和显示数据的应用。
...
Hydra是ILLA 云平台内的强大集成，它无缝连接和合并各种数据来源和应用程序。 Hydra作为一个中央枢纽，能够在不同系统之间进行高效率的数据流和通信，以便能够进行实时数据同步、自动化工作流和简化流程。 Hydra 凭借其强大的能力，使企业能够优化其业务，加强合作。 并轻松地做出以数据为驱动的决策，所有这些都是在国际利比里亚解放军云层的综合生态系统中。
...
| CA 证书              | 必填  | 用来核实服务器身份和确定安全的数字证书， 已加密客户端和 Hydra集群之间的连接。 如果您没有指定它，我们已经为您填写了默认证书。                                                                                                                                                        |
