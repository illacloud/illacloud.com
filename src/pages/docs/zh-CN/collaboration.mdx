---
title: 和成员协作
metaTitle: ILLA协作
description: 开发者可以通过分享链接同时在ILLA Builder工作。 开发者可以实时编辑并与其他人沟通，以便及时提供反馈，这将大大提高发展效率。
tagCategory: doc_menu_cooperation_click
---

* * *

## <Heading hidden>和成员协作</Heading>

开发者可以通过分享链接同时在ILLA Builder工作。 开发者可以通过邀请成员，和成员协作开发并获取实时的反馈，可以大幅提升开发效率

### 邀请成员

团队的所有者和管理员可以直接点击下图所示的按钮邀请成员。 

<img src='https://cdn.illacloud.com/official-website/img/docs/assemble/collaboration.png' alt="" />

<img src='https://cdn.illacloud.com/official-website/img/docs/assemble/collaboration(1).png' alt="" />

### 管理成员

<img src='https://cdn.illacloud.com/official-website/img/docs/assemble/collaboration(2).png' alt="" />

编辑器可以邀请或管理编辑器和观众，观众可以邀请或管理观众。 编辑者可以管理编辑者或浏览者，浏览者可以管理浏览者。如果您不希望编辑者和浏览者管理成员或访问成员列表，请关闭上图所示开关。

### 在同一个应用中协同开发

我们允许多个成员同时编辑一个应用。您可以实时看到哪些成员在和您协作，哪些组件在被编辑，如下图所示 您可以看到正在编辑哪些组件。 见下文。 

<img src='https://cdn.illacloud.com/official-website/img/docs/assemble/collaboration(3).png' alt="" />
