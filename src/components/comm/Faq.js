...
export const FAQ = ({ translationSpace = 'home' }) => {
  ...
  return (
    <div className={style.faqContainer}>
      ...
      <div className={style.faqListContainer}>
        {
          Array.isArray(content) && content.length > 0 && content.map(({ question, answer }, index) => (
            <Fragment key={question}>
              <div onClick={() => { showDesc(index) }} className={clsx(style.faqListQuestion, showIndex === index ? `${listStyle['activeBg']}` : ``)}>
                <span className='font-[500] text-[14px] leading-[22px] lg:text-[16px]'>{question}</span>
