...
export function ContentItem({ contentNode, img }) {
  ...
  useEffect(() => {
    if (ref.current.getBoundingClientRect().top < window.innerHeight) {
      ...
      console.log(
        'ContentItem top',
        top,
        (window.innerHeight - ref.current.getBoundingClientRect().top) / (window.innerHeight + 266)
      )
      setBgTop(top)
    }
  ...
  return (
    <div className="flex sm:h-[100vh] sm:overflow-y-hidden  flex-col sm:flex-row items-center bg-black justify-center pt-[40px] sm:pt-0 text-[28px] sm:text-[48px] font-bold text-white px-[48px] sm:px-0">
      ...
      <div className=" relative sm:h-[1110px] flex items-center sm:pl-[42px] ">
        <img
        ...
        <div
          style={{ top: bgTop }}
          className={clsx(
            'hidden sm:block h-[1110px] w-[558px] rounded-[40px]  bg-white/[0.12] absolute '
          )}
        />
      </div>
    </div>
