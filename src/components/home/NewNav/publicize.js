          ...
          <span className="flex flex-row justify-between">
            <span className={style.publicIntru}>
              ...
              <span>
                ...
                <span className='inline-block w-[45px] text-center'>{stars || DefaultStars}</span>
                <span>{t('title.stars')}</span>
              </span>
            </span>
          </span>
          <span className={style.publicSlogan} role='img' aria-label="star us">{t('title.star-us')} 👉</span>
        </span>
      </div>
    </Link>
  )
