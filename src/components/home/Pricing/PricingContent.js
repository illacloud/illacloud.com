        ...
        {activeList.map(
          (
            ...
            <div
              className={clsx(
                style.card,
                index === 2 ? style.lightCard : style.darkCard,
              )}
              key={`${title}${index}`}
            >
              <div onClick={() => setActiveBtn(!activeBtn)} className='cursor-pointer'>
                <span
                  className={clsx(style.cardTitle, titleColor ?? 'text-white')}
                >
                  {t(title)}
                  {earlyBird && (
                ...
                <div className="flex flex-row gap-[16px] items-center">
                  <div className={style.cardPriceContent}>
                    <span className={style.cardPrice}>
                      {t(editor.price)}
                      <span className={style.priceExtra}>{t('cycle-month')}</span>
                    </span>
