  ...
  return (
    <div className={style.mainContent}>
      ...
      <div className='flex flex-col items-center w-full gap-[40px] xl:gap-[72px]'>
        <div className={clsx(style.mainTextContainer, 'w-full xl:w-[580px] xl:text-center')}>
            ...
            deploy.items.map(({ icon, title, content, moreLink, moreTitle, clickCategory, showCategory }, index) => (
              <div className={style.deployItem} key={title}>
                ...
                <div className={clsx('flex flex-col items-start', content ? 'gap-[16px] xl:gap-[32px]' : 'gap-[8px] xl:gap-[16px]')}>
                  ...
                  {
                    t(content) && <div className='flex flex-col gap-[32px]' dangerouslySetInnerHTML={{ __html: t(content) }} />
                  }
                  <LearnMore href={moreLink} btnText={t(moreTitle)} translationName='selfhost' onClick={() => onClick(clickCategory) } showReport={index % 2 === 0 ? showCategory : undefined }/>
                </div>
              </div>
            ))
          }
